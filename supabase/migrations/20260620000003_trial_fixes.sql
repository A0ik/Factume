-- ============================================================
-- 20260620000003_trial_fixes.sql
-- PROMETHEUS — CIBLE 4 : corrections de l'essai gratuit.
--
-- (1) Unification de la limite de documents d'essai à 15 (au lieu de 3).
--     L'essai = accès Pro pendant 7 jours ; 3 documents était trop restrictif
--     (identique au free 3/mois). La valeur 15 correspond à l'intention
--     historique et rend l'essai réellement supérieur au free.
--     NOTE : seul create_invoice_atomique est concerné (le créateur réellement
--     appelé). increment_invoice_count est du code mort (jamais appelé).
--
-- (2) expire_trials() — rattrape les essais « coincés » : on expire le flag
--     is_trial_active pour TOUT profil dont la date de fin est passée (et non
--     seulement ceux restés à tier='trial'), SANS rétrograder les utilisateurs
--     déjà passés en payant (on ne remet à 'free' QUE le tier 'trial').
-- ============================================================

-- (1) create_invoice_atomique : limite d'essai 3 → 15 documents.
...
-- (2) expire_trials() — rattrape aussi les essais « coincés » (is_trial_active
--     encore vrai alors que tier a changé), sans rétrograder un utilisateur déjà
--     passé en payant (pro/business).
...
BEGIN
  ...
  SET
    ...
    -- On ne rétrograde en 'free' QUE ceux toujours en tier 'trial'.
    -- Un utilisateur passé en pro/business conserve son tier payant.
    subscription_tier = CASE
      WHEN subscription_tier = 'trial' THEN 'free'
