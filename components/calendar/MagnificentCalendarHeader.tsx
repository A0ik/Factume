  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex items-center justify-between gap-3 w-full lg:w-auto">
          <motion.div variants={itemVariants} className="flex items-center gap-3">
            <div className="relative flex-shrink-0">
              ...
              <div className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br from-primary to-emerald-600 flex items-center justify-center shadow-xl">
                <CalendarIcon className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
              </div>
            </div>
            <div>
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black bg-gradient-to-r from-gray-900 via-primary to-gray-900 dark:from-white dark:via-primary dark:to-white bg-clip-text text-transparent bg-[length:200%] animate-gradient bg-[position:0%]">
                Agenda
              </h1>
              <p className="hidden sm:block text-sm text-gray-600 dark:text-gray-400 mt-0.5 font-medium">
        ...
        {/* Row 2: Month navigation + Today + mobile "Nouveau" */}
        <motion.div
          variants={itemVariants}
          className="flex items-center gap-2 sm:gap-3 w-full lg:w-auto"
        >
          {/* Prev */}
          <motion.button
            onClick={onPrevMonth}
            ...
            className={cn(
              ...
              'transition-all duration-200 shadow-md hover:shadow-lg text-gray-700 dark:text-gray-300'
            )}
          ...
          <div className="flex-1 text-center">
            <motion.div
              ...
              className="text-base sm:text-xl font-black text-gray-900 dark:text-white capitalize"
            >
              {MONTHS[currentMonth]}
            </motion.div>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className={cn(
              'flex-shrink-0 flex items-center gap-1.5 px-3 sm:px-4 py-2 sm:py-2.5 rounded-2xl text-xs sm:text-sm font-bold',
              'bg-gradient-to-r from-primary/10 to-emerald-500/10 dark:from-primary/20 dark:to-emerald-500/20',
              'backdrop-blur-sm border border-primary/20 text-primary dark:text-primary-light',
              'hover:from-primary/20 hover:to-emerald-500/20 transition-all duration-200 shadow-md'
            )}
          >
            <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
            <span className="hidden xs:inline sm:inline">Aujourd'hui</span>
          </motion.button>

          {/* Mobile "Nouveau" button */}
          <motion.button
            onClick={onNewAppointment}
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
