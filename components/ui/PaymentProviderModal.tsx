      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="px-6 py-5 border-b border-gray-100 dark:border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-black text-gray-900 dark:text-white">Lien de paiement</h2>
              <p className="text-xs text-gray-500 dark:text-slate-400">Choisissez votre moyen de paiement</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 space-y-4">
          {/* Amount display */}
          <div className="text-center py-4 bg-gray-50 dark:bg-slate-800/50 rounded-xl">
            <p className="text-xs text-gray-400 uppercase tracking-wide font-semibold mb-1">Montant à payer</p>
            <p className="text-3xl font-black text-gray-900 dark:text-white">{fmtAmount}</p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className={`w-full relative p-4 rounded-xl border-2 transition-all ${
                !hasStripe
                  ? 'opacity-50 cursor-not-allowed border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-slate-800/50'
                  : 'border-indigo-200 bg-indigo-50/50 hover:border-indigo-400 hover:bg-indigo-50'
              }`}
            >
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                  hasStripe ? 'bg-indigo-100' : 'bg-gray-200 dark:bg-slate-700'
                }`}>
                ...
                <div className="flex-1 text-left">
                  ...
                  <div className="text-xs text-gray-500 dark:text-slate-400">Paiement sécurisé par carte</div>
                </div>
                {loading && selectedProvider === 'stripe' && (
                  <div className="animate-spin rounded-full h-5 w-5 border-2 border-indigo-600 border-t-transparent" />
                )}
                {!hasStripe && (
                  <span className="text-xs bg-gray-200 dark:bg-slate-700 text-gray-500 dark:text-slate-400 px-2 py-1 rounded-full font-medium">
                    Non configuré
                  </span>
                )}
              </div>
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                  hasSumUp ? 'bg-orange-100' : 'bg-gray-200 dark:bg-slate-700'
                }`}>
                  <Wallet size={28} className={hasSumUp ? 'text-orange-600' : 'text-gray-400'} />
                </div>
                <div className="flex-1 text-left">
                  <div className="font-bold text-gray-900 dark:text-white">SumUp</div>
                  <div className="text-xs text-gray-500 dark:text-slate-400">Paiement mobile simplifié</div>
                </div>
          ...
          {/* Info message */}
          {(!hasStripe || !hasSumUp) && (
            <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-800/50 rounded-xl p-3">
        ...
        {/* Footer */}
        <div className="px-6 py-4 bg-gray-50 dark:bg-slate-800/50 border-t border-gray-100 dark:border-white/10 flex gap-3">
          <button
            onClick={onClose}
            disabled={loading}
