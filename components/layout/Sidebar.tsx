...
export default function Sidebar() {
  ...
  const SubItem = ({ href, icon: Icon, label, locked, lockTier }: SubItem) => {
    ...
    if (locked) {
      return (
        <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-medium text-gray-300 dark:text-gray-600 cursor-not-allowed opacity-70 group" title={`Disponible avec ${lockTier === 'pro' ? 'Pro' : 'Business'}`}>
