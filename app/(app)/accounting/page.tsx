...
import {
  ...
  FileText, Zap, TrendingDown, BarChart2, AlertCircle, CheckCircle2,
  AlertTriangle, ExternalLink, Lock, Crown,
} from 'lucide-react';
import {
...
// ── Custom Tooltip for chart (dark mode aware) ──

function ChartTooltip({ active, payload, label }: any) {
  if (!active || !payload || !payload.length) return null;
  return (
    <div className="bg-white dark:bg-slate-800 p-3 rounded-xl border border-gray-100 dark:border-slate-700/50 shadow-xl">
      <p className="text-[11px] text-gray-500 dark:text-gray-400 font-semibold mb-1.5">{label}</p>
...
export default function AccountingPage() {
    ...
    return (
      <div className="space-y-6 max-w-2xl">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Abonnement requis</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 max-w-md">
            La comptabilité est disponible avec les plans Pro et Business. Passez à un plan supérieur pour débloquer cette fonctionnalité.
          </p>
          <Link
  ...
  return <AccountingContent currentYear={currentYear} currentQ={currentQ} />;
}

// ── Inner Content (after paywall passed) ──

function AccountingContent({ currentYear, currentQ }: { currentYear: number; currentQ: number }) {
  const { profile, user } = useAuthStore();
  const { invoices } = useDataStore();
  ...
  const bestMonth = useMemo(() => {
    ...
    return {
      label: new Date(key + '-01').toLocaleString('fr-FR', { month: 'long', year: 'numeric' }),
      amount,
    };
  }, [annualPaidInvoices]);
  ...
  const handleExportTVA = () => {
    ...
    const expRows = expenses
      ...
      .map(
        (e) =>
          `<tr><td>${e.vendor}</td><td>${new Date(e.date).toLocaleDateString('fr-FR')}</td><td>${formatCurrency(e.amount)}</td><td>${formatCurrency(e.vat_amount || 0)}</td></tr>`,
      )
      .join('');
  ...
  const tabs: { id: 'tva' | 'urssaf' | 'annual'; label: string; icon: typeof Calculator }[] = [
  ...
  // ── Expense helpers for quarterly table ──
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            className="text-sm font-semibold border border-gray-200 dark:border-slate-700 rounded-xl px-3 py-2 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/20"
          >
            {yearOptions.map((y) => (
        ...
        <div className="flex gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs font-bold text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700 px-3 py-2 rounded-xl transition-colors"
          >
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-gray-100 dark:bg-slate-800/50 rounded-2xl p-1 w-fit">
      ...
      {activeTab === 'tva' && (
        <div className="space-y-4">
          {/* Auto-entrepreneur info banner */}
          {isAE && (
            <div className="flex items-start gap-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/30 rounded-2xl p-4">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700 dark:text-amber-500 mt-0.5">
                  En tant qu&apos;auto-entrepreneur sous le seuil de franchise, vous n&apos;êtes pas
                  assujetti à la TVA. Si vous avez dépassé le seuil, consultez votre
                  expert-comptable.
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {/* TVA Collectée - primary gradient */}
            <div className="relative overflow-hidden bg-gradient-to-br from-primary to-primary-dark dark:from-primary dark:to-primary-dark rounded-2xl p-5 text-white">
              ...
              <p className="text-xs text-white/50 mt-1">
                {QUARTERS.find((q) => q.value === selectedQ)?.label} {selectedYear}
              </p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800/50 rounded-2xl p-5 border border-gray-100 dark:border-slate-700/50 shadow-sm">
              ...
              <p className="text-2xl font-black text-gray-900 dark:text-white">{formatCurrency(tvaDeductible)}</p>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">sur dépenses du trimestre</p>
            </div>

            {/* TVA à payer - conditional green/red */}
            <div
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <div
                  className={`w-6 h-6 rounded-lg flex items-center justify-center ${
                    tvaAPayer > 0 ? 'bg-red-100 dark:bg-red-500/20' : 'bg-green-100 dark:bg-green-500/20'
                  }`}
                >
                  <CheckCircle2
                    size={12}
                    className={tvaAPayer > 0 ? 'text-red-500' : 'text-green-500'}
                  />
                </div>
                <p
              ...
              <p
                className={`text-2xl font-black ${
                  tvaAPayer > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-700 dark:text-green-400'
                }`}
              >
              ...
              <p
                className={`text-xs mt-1 ${
                  tvaAPayer > 0 ? 'text-red-400 dark:text-red-500' : 'text-green-500 dark:text-green-500'
                }`}
              >
            ...
            {tvaCreditReportable > 0 ? (
              ...
              <div className="bg-white dark:bg-slate-800/50 rounded-2xl p-5 border border-gray-100 dark:border-slate-700/50 shadow-sm">
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-6 h-6 rounded-lg bg-gray-50 dark:bg-slate-700 flex items-center justify-center">
                ...
                <p className="text-2xl font-black text-gray-900 dark:text-white">
                  {paidInPeriod.reduce((s, i) => s + i.subtotal, 0) > 0
                    ? (
                        (tvaCollectee / paidInPeriod.reduce((s, i) => s + i.subtotal, 0)) *
                        100
                      ).toFixed(1)
                    : '0.0'}
                  %
                </p>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">taux TVA moyen</p>
              </div>
            )}
          </div>

          {/* Invoice table */}
          <div className="bg-white dark:bg-slate-800/50 rounded-2xl border border-gray-100 dark:border-slate-700/50 shadow-sm overflow-hidden">
            <div className="px-5 py-3.5 border-b border-gray-50 dark:border-slate-700/50 flex items-center justify-between">
              <p className="text-sm font-bold text-gray-700 dark:text-gray-300">
                Factures payées — {QUARTERS.find((q) => q.value === selectedQ)?.label}{' '}
                {selectedYear}
              </p>
              <span className="text-xs text-gray-400 dark:text-gray-500">
                {paidInPeriod.length} facture{paidInPeriod.length !== 1 ? 's' : ''}
              </span>
            </div>
            {paidInPeriod.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 dark:bg-slate-700/30 text-gray-500 dark:text-gray-400 text-xs font-semibold uppercase tracking-wide">
                  ...
                  <tbody className="divide-y divide-gray-50 dark:divide-slate-700/30">
                    {paidInPeriod.map((inv) => (
                      ...
                      >
                        <td className="px-5 py-3 font-mono text-xs font-semibold text-gray-700 dark:text-gray-300">
                          {inv.number}
                        </td>
                        <td className="px-5 py-3 text-gray-700 dark:text-gray-300 truncate max-w-[160px]">
                        ...
                        <td className="px-5 py-3 text-right text-primary font-semibold">
                  ...
                  <tfoot>
                    <tr className="bg-gray-50 dark:bg-slate-700/30 font-bold text-gray-800 dark:text-gray-300">
                      <td colSpan={3} className="px-5 py-3 text-xs uppercase tracking-wide">
                        Total
                      </td>
                      <td className="px-5 py-3 text-right">
                        {formatCurrency(paidInPeriod.reduce((s, i) => s + i.subtotal, 0))}
                      </td>
                      <td className="px-5 py-3 text-right text-primary">
                        {formatCurrency(tvaCollectee)}
                      </td>
                      <td className="px-5 py-3 text-right">
                        {formatCurrency(paidInPeriod.reduce((s, i) => s + i.total, 0))}
                      </td>
                    </tr>
                  </tfoot>
                </table>
          ...
          {/* Expenses TVA table */}
          {expenses.filter((e) => inQuarterYear(e.date, selectedYear, selectedQ) && e.vat_amount > 0)
            .length > 0 && (
            <div className="bg-white dark:bg-slate-800/50 rounded-2xl border border-gray-100 dark:border-slate-700/50 shadow-sm overflow-hidden">
              <div className="px-5 py-3.5 border-b border-gray-50 dark:border-slate-700/50">
                <p className="text-sm font-bold text-gray-700 dark:text-gray-300">
                    ...
                    {expenses
                      ...
                      .map((e) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-3 text-gray-500 dark:text-gray-400 text-xs capitalize">
                            {e.category}
                          </td>
                          <td className="px-5 py-3 text-gray-500 dark:text-gray-400 text-xs">
        ...
        <div className="space-y-4">
          {isAE ? (
            <>
              ...
              {/* Threshold warning */}
              {aeWarning && (
                <div className="flex items-start gap-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/30 rounded-2xl p-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-red-600 dark:text-red-500 mt-0.5">
                      Votre CA annuel ({formatCurrency(annualCA)}) approche du seuil de{' '}
                      {formatCurrency(AE_THRESHOLD_SERVICES)}. Au-delà, vous perdez le statut
                      auto-entrepreneur.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <p className="text-sm font-bold text-gray-900 dark:text-white">
                      Seuil auto-entrepreneur {selectedYear}
                    </p>
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                      Services BNC/BIC : {formatCurrency(AE_THRESHOLD_SERVICES)}
                    </p>
                  </div>
                  <p
                    className={`text-2xl font-black ${
                      ...
                      <div>
                        ...
                        <p
                          className={`text-lg font-black ${
                            q.value === selectedQ ? 'text-white' : 'text-gray-900 dark:text-white'
                          }`}
                        >
                          {formatCurrency(q.ca)}
                        </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500">
                    Indicatif uniquement, consultez votre expert-comptable
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-50 dark:bg-slate-700/30 rounded-xl p-4">
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">CA annuel HT</p>
      ...
      {activeTab === 'annual' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {/* CA total HT - dark gradient */}
            <div className="col-span-2 lg:col-span-1 relative overflow-hidden bg-gradient-to-br from-gray-900 to-gray-800 dark:from-slate-800 dark:to-slate-900 rounded-2xl p-5 text-white">
              ...
              <p className="text-xs text-white/40 mt-1">
                {selectedYear} &middot; {annualPaidInvoices.length} factures
              </p>
            </div>
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                >
                  <Zap
                    size={12}
                    className={resultatBrut >= 0 ? 'text-green-500' : 'text-red-500'}
                  />
                </div>
                <p
                  className={`text-xs font-semibold uppercase tracking-wide ${
              ...
              <p
                className={`text-xl font-black ${
                  resultatBrut >= 0 ? 'text-green-700 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                }`}
              >
                {formatCurrency(resultatBrut)}
              </p>
              <p
                className={`text-xs mt-1 ${
                  resultatBrut >= 0 ? 'text-green-500 dark:text-green-500' : 'text-red-400 dark:text-red-500'
                }`}
              >
                CA − charges
              </p>
            ...
            <div className="bg-white dark:bg-slate-800/50 rounded-2xl p-5 border border-gray-100 dark:border-slate-700/50 shadow-sm">
              ...
              <p className="text-xl font-black text-gray-900 dark:text-white">
                {formatCurrency(avgMonthlyCA)}
              </p>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                {bestMonth ? `↑ ${bestMonth.label}` : 'aucune donnée'}
              </p>
            </div>
          </div>

          {/* Monthly chart */}
          <div className="bg-white dark:bg-slate-800/50 rounded-2xl border border-gray-100 dark:border-slate-700/50 shadow-sm p-5">
            <div className="flex items-center gap-2 mb-5">
              <BarChart2 size={16} className="text-primary" />
              <div>
                <h2 className="font-bold text-gray-900 dark:text-white text-sm">
                  CA vs Charges — mensuel {selectedYear}
                </h2>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                  Chiffre d&apos;affaires HT encaissé vs dépenses
                </p>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-slate-800/50 rounded-2xl border border-gray-100 dark:border-slate-700/50 shadow-sm overflow-hidden">
            <div className="px-5 py-3.5 border-b border-gray-50 dark:border-slate-700/50">
              <p className="text-sm font-bold text-gray-700 dark:text-gray-300">
                Récapitulatif trimestriel {selectedYear}
              </p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-slate-700/30">
                  {quarterlyCA.map((q, idx) => {
                    const qExp = quarterlyExpenses[idx];
                    const qTva = quarterlyTVA[idx];
                    const res = q.ca - qExp - q.cotisation;
                    return (
                      <tr
                        key={q.value}
                        className={`hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors ${
                          q.value === selectedQ ? 'bg-primary/5 dark:bg-primary/10' : ''
                        }`}
                      >
                        <td className="px-5 py-3 font-semibold text-gray-900 dark:text-white">
                          {q.label}
                          {q.value === selectedQ && (
                            <span className="ml-2 text-[10px] text-primary font-bold">(actuel)</span>
                          )}
                        </td>
                        <td className="px-5 py-3 text-right font-bold text-gray-900 dark:text-white">
                          {formatCurrency(q.ca)}
                        ...
                        <td
                          className={`px-5 py-3 text-right font-bold ${
                            res >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                          }`}
                        >
                          {formatCurrency(res)}
                        </td>
                ...
                <tfoot>
                  <tr className="bg-gray-50 dark:bg-slate-700/30 font-bold text-gray-800 dark:text-gray-300">
                    ...
                    <td className="px-5 py-3 text-right text-primary">
                      {formatCurrency(annualPaidInvoices.reduce((s, i) => s + (i.vat_amount || 0), 0))}
                    </td>
                    <td className="px-5 py-3 text-right text-red-500 dark:text-red-400">
                      {formatCurrency(quarterlyCA.reduce((s, q) => s + q.cotisation, 0))}
                    </td>
                    <td
                      className={`px-5 py-3 text-right ${
                        resultatBrut >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                      }`}
                    >
                      {formatCurrency(
                        resultatBrut - quarterlyCA.reduce((s, q) => s + q.cotisation, 0),
                      )}
                    </td>
                  </tr>
                </tfoot>
