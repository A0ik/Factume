...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-gray-100 border border-gray-300 p-3 rounded-lg text-xs">
        <p className="text-slate-400 font-medium mb-1 capitalize">{label}</p>
        {payload.map((entry: any, i: number) => (
          <div key={i} className="flex items-center gap-2">
            <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: entry.color }} />
            <span className="text-gray-900 dark:text-white font-semibold">{entry.name}: {formatCurrency(entry.value)}</span>
          </div>
        ))}
      </div>
    );
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-5">
                ...
                >
                  <div className="flex gap-2 pt-3 pb-1 overflow-x-auto scrollbar-none">
                    {quickActions.map(({ href, icon: Icon, label, color, bg }) => (
                      <Link key={href} href={href} className="group flex-shrink-0 flex flex-col items-center gap-2 p-3 rounded-xl bg-gray-100 border border-gray-200 hover:border-gray-300 transition-colors active:scale-95 min-w-[72px]">
          ...
          <motion.div variants={itemVariants} className="hidden lg:flex items-center justify-between">
            <div>
              ...
              <h2 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white mt-0.5">
          ...
          <motion.div variants={itemVariants} className="lg:hidden flex items-center justify-between">
          ...
          <motion.div variants={itemVariants}>
            ...
            <div className="flex gap-2 lg:hidden overflow-x-auto scrollbar-none pb-1">
              <div className="flex-shrink-0 bg-gray-100 border border-gray-200 rounded-2xl p-3.5 min-w-[130px]">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">CA ce mois</p>
                <p className="text-lg font-bold text-gray-900 dark:text-white">{formatCurrency(stats?.mrr || 0)}</p>
              ...
              <Link href="/documents/factures?status=overdue" className="flex-shrink-0 min-w-[130px]">
                <motion.div whileTap={{ scale: 0.98 }} className={cn(
                  ...
                  overdueCount ? "bg-red-500/5 border-red-500/20" : "bg-gray-100 border-gray-200"
                )}>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">En retard</p>
                  <p className={cn("text-lg font-bold", overdueCount ? "text-red-400" : "text-gray-900 dark:text-white")}>{overdueCount}</p>
              ...
              <div className="flex-shrink-0 bg-gray-100 border border-gray-200 rounded-2xl p-3.5 min-w-[130px]">
                ...
                <p className="text-lg font-bold text-gray-900 dark:text-white">{dso}<span className="text-xs text-slate-500 ml-1">jours</span></p>
            ...
            <div className="hidden lg:grid grid-cols-4 gap-3">
              ...
              {[
                ...
                { label: 'En retard', value: stats?.overdueCount || 0, sub: 'factures', icon: AlertTriangle, color: stats?.overdueCount ? 'text-red-400' : 'text-slate-500', bg: stats?.overdueCount ? 'bg-red-500/10' : 'bg-gray-50', href: '/documents/factures?status=overdue' },
                ...
                <Link key={label} href={href} className="block">
                  <motion.div whileTap={{ scale: 0.98 }} className="bg-gray-100 border border-gray-200 rounded-2xl p-4 h-full">
          ...
          <motion.div variants={itemVariants} className="hidden lg:block bg-gray-50 border border-gray-200 rounded-2xl p-4">
            ...
            <div className="grid grid-cols-5 gap-2">
              {quickActions.map(({ href, icon: Icon, label, color, bg }) => (
                <Link key={href} href={href} className="group flex flex-col items-center gap-2.5 p-4 rounded-xl bg-gray-100 border border-gray-200 hover:border-gray-300 transition-colors">
          ...
          {/* Chart */}
          <motion.div variants={itemVariants} className="bg-gray-50 border border-gray-200 rounded-2xl p-4 overflow-hidden">
                ...
                {([1, 3, 6, 12] as const).map((p) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-md text-[10px] font-bold transition-all ${
                      period === p ? 'bg-gray-100 text-gray-900 dark:text-white' : 'text-slate-500 hover:text-slate-300'
                    }`}
                  >
                    {p}M
                  </button>
          ...
          <motion.div variants={itemVariants} className="bg-gray-50 border border-gray-200 rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 cursor-pointer">
              <h2 className="text-sm font-semibold text-gray-900 dark:text-white flex items-center gap-1.5">
            ...
            {recentInvoices.length === 0 ? (
              <div className="text-center py-10 px-4">
                ...
                <p className="text-xs text-gray-400 mt-0.5 mb-4">Créez votre première facture</p>
                <Link href="/invoices/new" className="inline-flex items-center gap-1.5 bg-emerald-500 hover:bg-emerald-400 text-white text-xs font-semibold px-4 py-2 rounded-lg hover:bg-emerald-400 transition-colors active:scale-95">
                  <Plus size={13} /> Créer
                </Link>
              </div>
            ) : (
              <div className="divide-y divide-gray-200">
                {recentInvoices.map((inv, i) => (
                  ...
                  >
                    <Link href={`/invoices/${inv.id}`} className="flex items-center gap-3 px-4 py-3.5 hover:bg-gray-100 transition-colors group active:bg-white/[0.05] cursor-pointer">
                      ...
                      <div className="text-right flex-shrink-0">
                        <p className={cn(
                          "text-sm font-bold",
                          inv.status === 'paid' ? 'text-emerald-400' : inv.status === 'overdue' ? 'text-red-400' : 'text-gray-900 dark:text-white',
                        )}>
