...
export default function NewLivraisonPage() {
  ...
  const docType: DocumentType = 'delivery_note';
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/ai/generate-invoice', {
        ...
        body: JSON.stringify({
          prompt: hasContent ? `MODIFICATION DE LIVRAISON:\n${aiPrompt}` : aiPrompt,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if ((e as Error).message === '__timeout__') {
        ...
        console.error('[new delivery note] createInvoice error:', e);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5 pb-8">
      <div className="fixed inset-0 pointer-events-none overflow-hidden -z-10">
        <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-indigo-500/[0.02] dark:bg-indigo-500/[0.03] rounded-full blur-3xl" />
        <div className="absolute top-1/3 right-0 w-[300px] h-[300px] bg-cyan-500/[0.02] dark:bg-cyan-500/[0.03] rounded-full blur-3xl" />
      </div>
      ...
      >
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={() => router.push('/documents/livraisons')}
        ...
        <div className="flex-1">
          <h1 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white">Nouveau bon de livraison</h1>
          <p className="text-xs sm:text-sm text-gray-400 dark:text-gray-500 mt-0.5">Remplissez les informations ci-dessous pour créer votre bon de livraison.</p>
        </div>
      </motion.div>
        ...
        {mode === 'voice' && (
          ...
          >
            {!sub.canUseVoice && (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-600 dark:text-amber-500/80 mt-0.5">Dicttez vos livraisons à voix haute, l'IA les remplit automatiquement</p>
                </div>
              </motion.button>
            )}
            ...
            <div className="flex flex-col items-center gap-4">
              <div className="relative">
                ...
                <motion.button
                  ...
                  disabled={loading}
                  className={`relative w-24 sm:w-28 h-24 sm:h-28 rounded-full flex items-center justify-center transition-all duration-200 shadow-xl ${recording ? 'bg-gradient-to-br from-red-500 to-red-600 hover:from-red-600 hover:to-red-700' : 'bg-gradient-to-br from-indigo-500 to-cyan-500 hover:from-indigo-600 hover:to-cyan-600'} ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  {loading ? (
              ...
              <div className="space-y-1 text-center">
                ...
                {!recording && !loading && (
                  <p className="text-xs text-gray-300 dark:text-gray-600">
                    Ex: "Bon de livraison pour Entreprise XYZ, design web 5 jours à 2000€, TVA 20%"
                  </p>
                )}
              </div>
            </div>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={() => setMode('manual')}
              className="text-sm text-indigo-600 font-semibold hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors"
            >
          ...
          >
            {!sub.canUseVoice && (
              ...
              >
                ...
                <div>
                  <p className="text-sm font-bold text-amber-800 dark:text-amber-400">Disponible avec Solo ou Pro</p>
                  <p className="text-xs text-amber-600 dark:text-amber-500/80 mt-0.5">Décrivez votre bon de livraison en texte, l'IA remplit tout</p>
                </div>
              </motion.button>
            )}
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sm font-bold text-gray-900 dark:text-white">Décrivez votre bon de livraison</h3>
              </div>
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-3">
                Ex: "Bon de livraison pour Entreprise XYZ pour développement web, 5 jours à 2000€ HT, TVA 20%"
              </p>
              <textarea
                value={aiPrompt}
                onChange={(e) => setAiPrompt(e.target.value)}
                rows={4}
                className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-white/10 text-sm resize-none bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 dark:focus:ring-indigo-500 dark:focus:border-indigo-400 transition-all"
                placeholder="Décrivez votre bon de livraison en langage naturel..."
            ...
            <motion.button
              ...
              className="w-full flex items-center justify-center gap-2 py-3 sm:py-3.5 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-500 hover:from-indigo-600 hover:to-cyan-600 text-white text-sm font-bold transition-all disabled:opacity-40 shadow-lg shadow-indigo-500/20"
            >
              {aiLoading ? (
                <>
                  <Loader2 size={16} className="animate-spin" />
                  L'IA génère votre bon de livraison...
                </>
              ) : (
                <>
                  <Sparkles size={16} />
            ...
            <div className="lg:col-span-2 space-y-3">
              ...
              <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm" style={{ overflow: 'visible' }}>
                ...
                <div className="p-4">
                    ...
                    {showSuggestions && suggestions.length > 0 && (
                      ...
                      >
                        {suggestions.slice(0, 5).map((c) => (
                          ...
                          >
                            <div className="w-7 h-7 rounded-lg bg-indigo-500/10 dark:bg-indigo-500/20 flex items-center justify-center text-indigo-500 text-xs font-bold flex-shrink-0">
                  ...
                  {clientId && (
                    <p className="text-xs text-indigo-600 dark:text-indigo-400 flex items-center gap-1 mt-2">
                ...
                <div className="p-4 space-y-2">
                  {items.map((item, idx) => (
                    <motion.div
                      ...
                      className="group relative p-3 sm:p-4 bg-gray-50 dark:bg-slate-800/50 rounded-xl border border-gray-100 dark:border-white/5 hover:border-indigo-500/20 dark:hover:border-indigo-500/30 transition-colors"
                    >
                      ...
                      <div className="flex gap-2 mb-2">
                        ...
                        <motion.button
                          ...
                          className="flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-indigo-500 to-cyan-500 hover:from-indigo-600 hover:to-cyan-600 text-white rounded-xl text-xs font-semibold shadow-md hover:shadow-lg transition-all whitespace-nowrap"
                ...
                <div className="p-4">
                  ...
                  {profile?.payment_terms && !notes && (
                    <motion.button
                      ...
                      className="mt-2 text-xs text-indigo-600 font-semibold hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 hover:underline transition-colors"
                    >
              ...
              <div className="lg:sticky lg:top-6 space-y-3">
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      placeholder="0"
                      className="w-24 px-3 py-2 rounded-xl border border-gray-200 dark:border-white/10 text-sm text-center font-bold bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 dark:focus:ring-indigo-500/30 focus:border-indigo-500 transition-all"
                    />
                    ...
                    <div className="flex gap-1.5 mt-2">
                      {[5, 10, 15, 20].map((p) => (
                        <motion.button
                          ...
                          className="text-xs px-2 py-1 rounded-lg font-semibold transition-colors bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-gray-400 hover:bg-indigo-500/10 dark:hover:bg-indigo-500/20 hover:text-indigo-600 dark:hover:text-indigo-400"
                        >
                          {p}%
                        </motion.button>
                      ))}
                ...
                >
                  ...
                  <div className="px-5 py-4 space-y-2.5">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-cyan-400 font-black text-lg sm:text-xl tabular-nums">{formatCurrency(total)}</span>
                    </div>
                  </div>
                ...
                >
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-semibold text-gray-700 dark:text-gray-300">Bon de livraison</span>
                  </div>
                  <div className="flex justify-between text-xs">
                ...
                <motion.button
                  ...
                  onClick={handleSave}
                  disabled={saving}
                  className={`w-full flex items-center justify-center gap-2 py-3.5 sm:py-4 rounded-2xl text-sm font-bold transition-all shadow-xl bg-gradient-to-r from-indigo-500 to-cyan-500 text-white hover:from-indigo-600 hover:to-cyan-600 shadow-indigo-500/30 hover:shadow-indigo-500/40 ${saving ? 'cursor-not-allowed opacity-70' : ''}`}
                >
                  {saving ? (
                    <>
                      <Loader2 size={16} className="animate-spin" />
              ...
              <div className="p-4 border-b border-gray-200 dark:border-white/10">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
                  />
                </div>
              </div>
                  ...
                  <div className="text-center py-12">
                    ...
                    {!productSearch && (
                      <button
                        ...
                        className="mt-4 px-4 py-2 bg-gradient-to-r from-indigo-500 to-cyan-500 text-white rounded-xl text-sm font-semibold hover:shadow-lg transition-all"
                      >
                        Créer des produits
                      </button>
                    )}
                  ...
                  <div className="space-y-2">
                    {filteredProducts.map((product) => (
                      <motion.button
                        ...
                        className="w-full p-4 bg-gray-50 dark:bg-white/5 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 border border-gray-200 dark:border-white/10 hover:border-indigo-200 dark:hover:border-indigo-500/30 rounded-2xl transition-all text-left group"
                      >
