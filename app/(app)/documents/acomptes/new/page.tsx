...
export default function NewAcomptePage() {
  ...
  const docType: DocumentType = 'deposit';
  ...
  // Acompte-specific: linked invoice + percentage
  const [linkedInvoiceId, setLinkedInvoiceId] = useState<string | null>(null);
  const [showInvoicePicker, setShowInvoicePicker] = useState(false);
  const [depositPercent, setDepositPercent] = useState<number>(0);
  const userInvoices = invoices.filter((i) => i.document_type === 'invoice');
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/ai/generate-invoice', {
        ...
        body: JSON.stringify({
          prompt: hasContent ? `MODIFICATION D'ACOMPTE:\n${aiPrompt}` : aiPrompt,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if ((e as Error).message === '__timeout__') {
        ...
        console.error('[new deposit] createInvoice error:', e);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5 pb-8">
      <div className="fixed inset-0 pointer-events-none overflow-hidden -z-10">
        <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-emerald-500/[0.02] dark:bg-emerald-500/[0.03] rounded-full blur-3xl" />
        <div className="absolute top-1/3 right-0 w-[300px] h-[300px] bg-teal-500/[0.02] dark:bg-teal-500/[0.03] rounded-full blur-3xl" />
      </div>
      ...
      >
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={() => router.push('/documents/acomptes')}
        ...
        <div className="flex-1">
          <h1 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white">Nouvelle facture d'acompte</h1>
          <p className="text-xs sm:text-sm text-gray-400 dark:text-gray-500 mt-0.5">Remplissez les informations ci-dessous pour créer votre facture d'acompte.</p>
        </div>
      </motion.div>
      ...
      <AnimatePresence>
        {mode === 'voice' && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm"
          >
            <PulseVoiceRecorder
        ...
        {mode === 'ai' && (
          ...
          >
            {!sub.canUseVoice && (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-600 dark:text-amber-500/80 mt-0.5">Décrivez votre facture d'acompte en texte, l'IA remplit tout</p>
                </div>
              </motion.button>
            )}
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sm font-bold text-gray-900 dark:text-white">Décrivez votre facture d'acompte</h3>
              </div>
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-3">
                Ex: "Facture d'acompte pour Entreprise XYZ pour développement web, 5 jours à 2000€ HT, TVA 20%"
              </p>
              <textarea
                value={aiPrompt}
                onChange={(e) => setAiPrompt(e.target.value)}
                rows={4}
                className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-white/10 text-sm resize-none bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-300 focus:border-emerald-400 dark:focus:ring-emerald-500 dark:focus:border-emerald-400 transition-all"
                placeholder="Décrivez votre facture d'acompte en langage naturel..."
            ...
            <motion.button
              ...
              className="w-full flex items-center justify-center gap-2 py-3 sm:py-3.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white text-sm font-bold transition-all disabled:opacity-40 shadow-lg shadow-emerald-500/20"
            >
              {aiLoading ? (
                <>
                  <Loader2 size={16} className="animate-spin" />
                  L'IA génère votre facture d'acompte...
                </>
              ) : (
                <>
                  <Sparkles size={16} />
        ...
        {mode === 'manual' && (
          ...
          >
            <div className="lg:col-span-2 space-y-3">
                    ...
                    {showSuggestions && suggestions.length > 0 && (
                      ...
                      >
                        {suggestions.slice(0, 5).map((c) => (
                          ...
                          >
                            <div className="w-7 h-7 rounded-lg bg-emerald-500/10 dark:bg-emerald-500/20 flex items-center justify-center text-emerald-500 text-xs font-bold flex-shrink-0">
              ...
              {/* Acompte-specific: facture d'origine + pourcentage */}
              <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm">
                <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-slate-800/50">
                  <Banknote size={15} className="text-gray-400 dark:text-gray-500" />
                  <h3 className="text-sm font-bold text-gray-700 dark:text-gray-300">Acompte</h3>
                  {linkedInvoice && (
                    <span className="ml-auto text-xs text-teal-600 dark:text-teal-400 font-semibold">{depositPercent > 0 ? `${depositPercent}% de ${linkedInvoice.number}` : linkedInvoice.number}</span>
                  )}
                </div>
                <div className="p-4 space-y-3">
                  <div>
                    <label className="text-[11px] text-gray-400 dark:text-gray-500 font-medium block mb-1.5">Facture associée</label>
                    {linkedInvoice ? (
                      <div className="flex items-center gap-3 p-3 bg-teal-50 dark:bg-teal-500/10 rounded-xl border border-teal-100 dark:border-teal-500/20">
                        <div className="flex-1 min-w-0">
                        ...
                        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => { setLinkedInvoiceId(null); setDepositPercent(0); }} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 text-gray-400 hover:text-red-500 transition-colors">
                          <X size={14} />
                        </motion.button>
                      </div>
                    ) : (
                      <div className="relative">
                        <button onClick={() => setShowInvoicePicker(!showInvoicePicker)} className="w-full text-left px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 text-sm text-gray-400 dark:text-gray-500 bg-white dark:bg-slate-800 hover:border-teal-300 dark:hover:border-teal-500/30 transition-colors">
                        ...
                        {showInvoicePicker && (
                          <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-white dark:bg-slate-800 border border-gray-200 dark:border-white/10 rounded-xl shadow-xl max-h-64 overflow-y-auto">
                            {userInvoices.length === 0 ? (
                              ...
                              <button key={inv.id} className="w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors border-b border-gray-50 dark:border-white/5 last:border-0" onClick={() => {
                                ...
                                <div className="flex justify-between items-center">
                                  <div><p className="font-semibold text-gray-900 dark:text-white">{inv.number}</p><p className="text-xs text-gray-400">{inv.client?.name || inv.client_name_override}</p></div>
                  ...
                  {linkedInvoice && (
                    <div>
                      <label className="text-[11px] text-gray-400 dark:text-gray-500 font-medium block mb-1.5">Pourcentage d'acompte</label>
                      <div className="flex items-center gap-2">
                        <input type="number" min={1} max={100} step={1} value={depositPercent || ''} onChange={(e) => {
                          const pct = Math.min(100, Math.max(0, parseInt(e.target.value) || 0));
                          setDepositPercent(pct);
                          if (pct > 0 && linkedInvoice) {
                            const depositHT = linkedInvoice.subtotal * (pct / 100);
                            setItems([{ id: generateId(), description: `Acompte de ${pct}% sur ${linkedInvoice.number}`, quantity: 1, unit_price: Math.round(depositHT * 100) / 100, vat_rate: linkedInvoice.items?.[0]?.vat_rate || 20 }]);
                          }
                        }} placeholder="%" className="w-24 px-3 py-2 rounded-xl border border-gray-200 dark:border-white/10 text-sm text-center font-bold bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all" />
                        ...
                        <div className="flex gap-1">
                          {[25, 30, 50].map((p) => (
                            <button key={p} onClick={() => {
                              setDepositPercent(p);
                              if (linkedInvoice) {
                                const depositHT = linkedInvoice.subtotal * (p / 100);
                                setItems([{ id: generateId(), description: `Acompte de ${p}% sur ${linkedInvoice.number}`, quantity: 1, unit_price: Math.round(depositHT * 100) / 100, vat_rate: linkedInvoice.items?.[0]?.vat_rate || 20 }]);
                              }
                            }} className={`text-xs px-2 py-1 rounded-lg font-semibold transition-colors ${depositPercent === p ? 'bg-teal-100 dark:bg-teal-500/20 text-teal-700 dark:text-teal-400' : 'bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-gray-400 hover:bg-teal-500/10'}`}>
                              {p}%
                            </button>
                          ))}
                        </div>
                      ...
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                        Montant acompte HT : {formatCurrency(linkedInvoice.subtotal * (depositPercent / 100))} — Solde restant : {formatCurrency(linkedInvoice.subtotal * (1 - depositPercent / 100))}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="p-4 space-y-2">
                  {items.map((item, idx) => (
                    <motion.div
                      ...
                      className="group relative p-3 sm:p-4 bg-gray-50 dark:bg-slate-800/50 rounded-xl border border-gray-100 dark:border-white/5 hover:border-emerald-500/20 dark:hover:border-emerald-500/30 transition-colors"
                    >
                      ...
                      <div className="flex gap-2 mb-2">
                        ...
                        <motion.button
                          ...
                          className="flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white rounded-xl text-xs font-semibold shadow-md hover:shadow-lg transition-all whitespace-nowrap"
                ...
                <div className="p-4">
                  ...
                  {profile?.payment_terms && !notes && (
                    <motion.button
                      ...
                      className="mt-2 text-xs text-emerald-600 font-semibold hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 hover:underline transition-colors"
                    >
              ...
              <div className="lg:sticky lg:top-6 space-y-3">
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      placeholder="0"
                      className="w-24 px-3 py-2 rounded-xl border border-gray-200 dark:border-white/10 text-sm text-center font-bold bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 dark:focus:ring-emerald-500/30 focus:border-emerald-500 transition-all"
                    />
                    ...
                    <div className="flex gap-1.5 mt-2">
                      {[5, 10, 15, 20].map((p) => (
                        <motion.button
                          ...
                          className="text-xs px-2 py-1 rounded-lg font-semibold transition-colors bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-gray-400 hover:bg-emerald-500/10 dark:hover:bg-emerald-500/20 hover:text-emerald-600 dark:hover:text-emerald-400"
                        >
                          {p}%
                        </motion.button>
                      ))}
                ...
                >
                  ...
                  <div className="px-5 py-4 space-y-2.5">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-teal-400 font-black text-lg sm:text-xl tabular-nums">{formatCurrency(total)}</span>
                    </div>
                  </div>
                ...
                >
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-semibold text-gray-700 dark:text-gray-300">Facture d'acompte</span>
                  </div>
                  <div className="flex justify-between text-xs">
                ...
                <motion.button
                  ...
                  onClick={handleSave}
                  disabled={saving}
                  className={`w-full flex items-center justify-center gap-2 py-3.5 sm:py-4 rounded-2xl text-sm font-bold transition-all shadow-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white hover:from-emerald-600 hover:to-teal-600 shadow-emerald-500/30 hover:shadow-emerald-500/40 ${saving ? 'cursor-not-allowed opacity-70' : ''}`}
                >
                  {saving ? (
                    <>
                      <Loader2 size={16} className="animate-spin" />
              ...
              <div className="p-4 border-b border-gray-200 dark:border-white/10">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all"
                  />
                </div>
              </div>
                  ...
                  <div className="text-center py-12">
                    ...
                    {!productSearch && (
                      <button
                        ...
                        className="mt-4 px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl text-sm font-semibold hover:shadow-lg transition-all"
                      >
                        Créer des produits
                      </button>
                    )}
                  ...
                  <div className="space-y-2">
                    {filteredProducts.map((product) => (
                      <motion.button
                        ...
                        className="w-full p-4 bg-gray-50 dark:bg-white/5 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 border border-gray-200 dark:border-white/10 hover:border-emerald-200 dark:hover:border-emerald-500/30 rounded-2xl transition-all text-left group"
                      >
