...
export default function SharePage({ params }: { params: Promise<{ invoiceId: string }> }) {
  ...
  const isSignable = ['quote', 'delivery_note', 'purchase_order'].includes(invoice.document_type);
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-4">
        ...
        {signed && (
          <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-2xl p-4">
            ...
            {invoice.document_type === 'quote' ? 'Devis accepté et signé'
              : invoice.document_type === 'delivery_note' ? 'Bon de livraison signé'
              : invoice.document_type === 'purchase_order' ? 'Bon de commande signé'
              : 'Document signé'}
          </p>
        ...
        {/* ── E-Signature block (quotes, delivery notes, purchase orders) ── */}
        {isSignable && !signed && (
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-sm font-bold text-gray-900">
                  {invoice.document_type === 'quote' ? 'Signer ce devis'
                    : invoice.document_type === 'delivery_note' ? 'Signer le bon de livraison'
                    : 'Signer le bon de commande'}
                </p>
                <p className="text-xs text-gray-500">Votre signature vaut acceptation du document</p>
              </div>
            </div>
            ...
            {!showSignPad ? (
              ...
              <div className="space-y-4">
                ...
                <p className="text-[11px] text-gray-400 leading-relaxed">
                  En signant, vous acceptez les termes du document {invoice.number} ({docLabel}) d&apos;un montant de <strong>{fmt(invoice.total)}</strong>.
